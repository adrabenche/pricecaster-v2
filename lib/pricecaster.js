...
const ContractInfo = {
  ...
  mapper: {
    approvalProgramFile: 'teal/wormhole/build/mapper-approval.teal',
    clearStateProgramFile: 'teal/wormhole/build/mapper-clear.teal',
    compiledApproval: {
      bytes: undefined,
      hash: undefined
    },
...
class PricecasterLib {
  constructor (algodClient, ownerAddr = undefined) {
     ...
     */
    this.compileApprovalProgram = async function (contract, flags) {
      let program = fs.readFileSync(ContractInfo[contract].approvalProgramFile, 'utf8')
      program = program.replace(/TMPL_I_TESTING/g, flags.testmode ? '1' : '0')
      console.log(program)
     ...
     */
    this.createApp = async function (sender, contract, localInts, localBytes, globalInts, globalBytes, appArgs, flags, signCallback) {
      ...
      await this.compileApprovalProgram(contract, flags)
     ...
     * @return {String} transaction id of the created application
     */
    this.createVaaProcessorApp = async function (sender, gexpTime, gsindex, gkeys, flags, signCallback) {
      return await this.createApp(sender, 'vaaProcessor', 0, 0, 5, 21,
        [new Uint8Array(Buffer.from(gkeys, 'hex')),
          ...
          algosdk.encodeUint64(parseInt(gsindex))], flags, signCallback)
    }
    ...
    /**
       ...
       * @return {String} transaction id of the created application
       */
    this.createPricekeeperApp = async function (sender, vaaProcessorAppId, flags, signCallback) {
      return await this.createApp(sender, 'pricekeeper', 0, 0, 1, 63,
        [algosdk.encodeUint64(parseInt(vaaProcessorAppId))], flags, signCallback)
    }
    ...
    /**
       ...
       * @return {String} transaction id of the created application
       */
    this.createMapperApp = async function (sender, flags, signCallback) {
      return this.createApp(sender, 'mapper', 0, 0, 0, 64, [], flags, signCallback)
    }
    ...
    this.setMappingEntry = async function (sender, asaId, key, signCallback) {
      if (!algosdk.isValidAddress(sender)) {
        throw new Error('Invalid sender address: ' + sender)
      }
      const appArgs = []
      appArgs.push(new Uint8Array(Buffer.from('store')), algosdk.encodeUint64(asaId), key)
      return await this.callApp(sender, 'mapper', appArgs, [], signCallback)
    }
